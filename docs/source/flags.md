...
* OPTIONAL CL parameters
  * `--config configurationFilePath`
    * configuration preferences priority
      1. specified 
         1. | .yaml
         2. | config file
           * OPTIONAL
  ...
  * `--debug`
    * run ElastAlert 2 | debug mode
      * increase the logging verboseness
      * all alerts are changed -- to -- ``DebugAlerter`` /
        * prints alerts
        * suppresses alerts normal action
        * skips writing search & alert metadata back -- to -- Elasticsearch
